...
const AllAppsPage = () => {
  ...
  },[currentpage,sort,order,limit,search])

  const hendleselet = (e)=>{
    console.log(e.target.value)
    const sortitem = e.target.value;
    setsort(sortitem.split("-")[0]);
    setorder(sortitem.split("-")[1])
  }
      ...
      <div className="w-11/12 mx-auto flex flex-col-reverse lg:flex-row gap-5 items-start justify-between lg:items-end mt-10">
        ...
        <form>
          <label className="input max-w-[300px] w-[300px] input-secondary">
            ...
            <input onChange={hendlesearch} value={search}type="search" className="" placeholder="Search Apps" />
          </label>
        </form>

        <div className="">
          <select onChange={hendleselet} className="select bg-white">
